...
object BuildGenYaml extends BuildGen {
  ...
  override def writeBuildFiles(
    ...
    // Meta-build files require Scala syntax.
    val metaBuildFiles = BuildGenScala.writeMetaBuildFiles(
      baseDir,
      baseModule,
      depNames,
      metaMvnDeps
    ...
    val nestedBuildFiles = for (pkg <- nestedPackages) yield {
      val sub = os.sub / pkg.dir / "package.mill.yaml"
      println(s"writing $sub")
      val file = baseDir / sub
      os.write(file, renderYamlPackage(pkg, None, None, Nil))
      file
    }
  ...
  private def renderYamlModuleBody(module: ModuleSpec): Seq[String] = {
    ...
    renderYamlSource("checkstyleConfig", checkstyleConfig)
    renderYamlMvnDepsList("checkstyleMvnDeps", checkstyleMvnDeps)
    renderYamlStringListValuesPlain("checkstyleOptions", checkstyleOptions)
    renderYamlStringValue("checkstyleVersion", checkstyleVersion)

    renderYamlStringListValuesPlain("pmdOptions", pmdOptions)
    renderYamlStringValue("pmdVersion", pmdVersion)
  ...
  private def renderYamlSource(name: String, value: Value[String]): Seq[String] = {
    value.base.fold(Nil)(path => Seq(s"$name: $path"))
  }
  ...
  private def renderYamlPomSettings(value: Value[PomSettings]): Seq[String] = {
    value.base.fold(Nil) { pom =>
      ...
      if (contentLines.isEmpty) Seq("pomSettings: {}")
      else "pomSettings:" +: contentLines
    }
  }
